...
symbols = ['btcusdt', 'ethusdt', 'solusdt', 'bnbusdt', 'dogrusdt', 'wifusdt']
...
class TradeAggregator:
    ...
    async def check_and_print_trades(self):
        timestamp_ofnow = datetime.utcnow().strftime("%H:%M:%S")
        deletions = []
        for trade_key, usd_size in self.trade_buckets.items():
            symbol, second, is_buyer_maker = trade_key
            if second < timestamp_ofnow and usd_size > 500000:
                attrs = ['bold']
                ...
                if usd_size > 3000000:
                    ...
                    usd_size = usd_size / 100000
                    cprint(f"\033[5m{trade_type} {symbol} {second} ${usd_size:.2f}m", 'white', back_color, attrs=attrs)
